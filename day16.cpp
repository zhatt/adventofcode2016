...
std::vector<bool> stringToBits( const std::string& startBits ) {
    ...
    bits.reserve( startBits.size() );
    for ( auto c : startBits ) {
...
std::vector<bool> calculateChecksum( const std::vector<bool> bits ) {
    assert( bits.size() % 2 == 0 );
    std::vector<bool> csum( bits );
    while ( csum.size() && csum.size() % 2 == 0 ) {
        size_t destIndex = 0;
        for ( size_t sourceIndex = 0; sourceIndex < csum.size();
            sourceIndex += 2 ) {
            if ( csum[ sourceIndex ] == csum[ sourceIndex + 1 ] ) {
                csum[ destIndex ] = true;
            } else {
                csum[ destIndex ] = false;
            }
            destIndex ++;
        }

        csum.resize( destIndex );
    }
    return csum;
}
...
std::vector<bool> fillDisk( std::vector<bool> initialState, unsigned size ) {
    ...
    std::vector<bool> bits( initialState );
    while ( bits.size() < size ) {
        bits = dragonCurve( bits );
    }
    bits.resize( size );
    auto csum = calculateChecksum( bits );
    return csum;
}
...
int mainfunc( std::istream& is, std::ostream& os, Part part ) {
    ...
    unsigned length = part == Part::PART1 ? 272 : 35651584;
    auto csum = fillDisk( stringToBits( input ), length );
    for ( auto bit : csum ) {
        os << ( bit ? '1' : '0' );
    }
    os << std::endl;
